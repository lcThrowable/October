...
function mytime(){
	if(null!=investment&& investment=="W"){		
		if($("#notice2i1\\[1\\]").is(":visible")){
	        $('[value="Y"][id^="notice"]').parent().css({"display":"inline"});
			$('[value="N"][id^="notice"]').parent().css({"display":"inline"});
			$('[value="Y"][id^="notice"]').parent().css({"padding-left":"20px"});
			$('[value="N"][id^="notice"]').parent().css({"padding-left":"60px"});
			clearInterval(interval2);
		}
	}
}

var interval2=setInterval(mytime,300); //mytime是方法，1000表示每1秒执行一次
...
/*function minorInsu(){
	var b=parseInt(vueobj["testdivchange"].formdata.noticeminorInsu.noticeinfo1);
	var c=parseInt(vueobj["testdivchange"].formdata.noticeminorInsu.noticeinfo2);
	if(isNaN(c)){
		c=0;
	}
	if(isNaN(b)){
		b=0;
		  ...
		  if(maininsuAge<10||(twoinsuAge!=""&&twoinsuAge<10)){			
	    	var d=parseInt(200000-b-c);
	   		vueobj["testdivchange"].$set(vueobj["testdivchange"].formdata.noticeminorInsu,"noticeinfo3", d);	  
	       }else if((maininsuAge>=10&&maininsuAge<18)||(twoinsuAge!=""&&twoinsuAge>=10&&twoinsuAge<18)){		    
	    	var d=parseInt(500000-b-c);	    
		   	vueobj["testdivchange"].$set(vueobj["testdivchange"].formdata.noticeminorInsu,"noticeinfo3", d);	   			         
	       }	
}
